...
# ╔═╡ 3f4bc896-5981-11ee-107b-b92cd890a296
using Statistics, Plots, GLM, DataFrames

# ╔═╡ bbba4d72-8b55-4a30-a69f-4d8010c81253
# Do not edit this cell!!!
begin
	using HTTP
	r = HTTP.request("GET", "https://raw.githubusercontent.com/chem330/chem330.github.io/main/nitrate/nitrate-tests.jl")
	eval(Meta.parse(String(r.body)))
	inst_stocks=setstockconcs()
end;

# ╔═╡ fe60269c-6a24-44a2-8915-1993709dce07
using PlutoTeachingTools, PlutoUI

# ╔═╡ 7b93d62f-6575-4233-8202-97877c2d1295
ChooseDisplayMode()

# ╔═╡ 60925d32-0a6e-48b5-9b4d-d0d76b7ddfb1
md"""
...
# ╔═╡ c835f0e0-1d90-4d8c-ad3b-4f6d9c032070
calibration_fit = lm(@formula(A~C), std_data)

# ╔═╡ c15ebb5f-59c9-4007-b393-19f0ef3513f8
R² = r2(calibration_fit)

# ╔═╡ c8007f2e-65e9-42dd-8fe3-74803de9ab45
nitrate_cal = calPlot(std_data.C, std_data.A, coef(calibration_fit)[2], coef(calibration_fit)[1]);

# ╔═╡ 90e3aec6-ab65-4568-b1f7-fda4f574a7d8
begin
	plot(nitrate_cal)
	# finish this plot with labels, units, etc.
end

# ╔═╡ 8f406111-fceb-468b-a7fb-81d292db2fb4
md"## QC Validation"

# ╔═╡ b489c109-a4a8-49d2-a71b-3d66cf067ab7
qc_measured_conc = missing

# ╔═╡ a28d9653-28ac-4416-a300-0d85a7b28bdc
qc_percent_error =  missing

# ╔═╡ d7cfd965-7a43-4ab8-9ac5-225527618180
if !ismissing(qc_measured_conc) && !ismissing(calibration_fit)
	if isapprox(qc_percent_error, (check_conc(qc_A, coef(calibration_fit)[2], coef(calibration_fit)[1]) - inst_stocks.qc) / inst_stocks.qc * 100, atol = 0.01)
		correct()
	else
		keep_working(md"Make sure your answer is a percentage comparing your measured concentration to the known concentration.")
	end
end
...
if any(.!ismissing.(nitrate_mg))  && !ismissing(sample_A)
	if all(isapprox.(nitrate_mg, (check_conc.(sample_A, coef(calibration_fit)[2], coef(calibration_fit)[1])), atol = 0.001))
		correct(md"Your nitrate calculation is correct!")
	else
		hint(md"Convert your nitrate (NO₃) concentration to nitrogen (N).  Round the masses of nitrate and nitrogen to the hundreths place.")
	end
end
...
if any(.!ismissing.(nitrate_N)) && any(.!ismissing.(nitrate_mg))
	if all(isapprox.(nitrate_N, check_nitrate_n.(nitrate_mg), atol = 0.0001))
		correct(md"Your nitrate-N calculation is correct!")
	else
		hint(md"Convert your nitrate (NO₃) concentration to nitrogen (N).  Round the masses of nitrate and nitrogen to the hundreths place.")
	end
end
...
# insert a bar plot of your samples here
# show your nitrate (NO3) concentrations on the y-axis

# ╔═╡ 58e4ff0f-910e-48aa-a367-6f5efe119e99
md"""
# Conclusions

Write your conclusions below.  Here are some ideas of things to include:

- report your results in mg/L NO3 and mg/L N
- discuss the interpretation of the intercept, slope, and R² of your calibration curve
- discuss your nitrate results in relation to typical values, regulations, and/or health or environmental implications
"""
